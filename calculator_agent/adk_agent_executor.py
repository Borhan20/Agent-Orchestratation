...
class ADKAgentExecutor(AgentExecutor):
    ...
    async def _process_request(
        ...
        auth_details = None # Keep this if you re-enable auth, otherwise can remove
        async for event in self._run_agent(session_id, new_message):
            # REMOVE OR COMMENT OUT THIS LINE:
            # return "success"

            # UNCOMMENT AND USE THIS LOGIC:
            ...
            if event.is_final_response():
                ...
                task_updater.complete() # Mark task as complete
                break # Exit the loop as the response is final
            if not event.get_function_calls():
                ...
                # This 'else' block will catch the function calls you saw in the logs.
                # When there are function calls, the ADK runner will automatically
                # execute them and then pass the results back to the LLM for
                # further processing, so you don't typically need explicit
                # handling here *unless* you need to intercept and modify.
                logger.debug('Received function calls, continuing agent execution.')


        # if auth_details: # Keep this if you re-enable auth, otherwise can remove
    ...
    async def execute(
        ...
        logger.debug('[Calculator] execute exiting')
